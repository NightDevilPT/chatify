...
async function bootstrap() {
  ...
  loggerService.log(`Server is running : http://localhost:${(process.env.PORT ?? 5000)}/api`);
  loggerService.log(`Swagger is running : http://localhost:${(process.env.PORT ?? 5000)}/swagger`);
}
bootstrap();
